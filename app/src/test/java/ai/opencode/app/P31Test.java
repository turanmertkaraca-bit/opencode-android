...
public class P31Test {
    ...
    @Test
    public void theme_p45_claudePaletteRemoved() {
        // P45: the rejected P44 face is GONE — removed from PALETTES,
        // PALETTE_DATA and GRAD_DATA. Every lookup reads it as unknown:
        // the index floor and the name fallback, exactly like garbage.
        assertEquals(0, Theme.paletteIndex("claude"));
        assertEquals("OLED black", Theme.paletteName("claude"));
        for (String id : Theme.PALETTES)
            assertNotEquals("claude must not ship in the table", "claude", id);
        // paper is the one light palette now
        assertNull(Theme.lightPalette("claude"));
        assertEquals("paper", Theme.lightPalette("paper"));
        assertNull(Theme.lightPalette("graphite"));
    }
    ...
    public void theme_unknownId_fallsBackToOled() {
        ...
        // P45: the removed claude id reads as unknown
        assertEquals(0, Theme.paletteIndex("claude"));
    }
